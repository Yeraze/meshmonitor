...
| **Transport filter (Show RF / UDP / MQTT)** | Show/hide markers **and traceroute route segments** by how the traffic reached MeshMonitor — over the air (**RF**), the Meshtastic multicast-UDP local transport (**UDP**), or an **MQTT** broker/bridge. Mirrors the same toggle on the Dashboard/Nodes map. A node heard through more than one transport (across sources) stays visible as long as any of its transports is enabled. Since 4.16.0 the same rule applies to route segments — see [Transport filtering and route segments](/features/maps#transport-filtering-and-route-segments). |
...
- **Basemap** — the 3D view reuses whichever raster tileset you have selected for the 2D map, and (New in 4.14) the **tileset selector is available in 3D too**, so you can switch basemaps without dropping back to 2D. If your selected tileset is **vector-only**, 3D can't drape a vector style over terrain yet, so it substitutes the default OpenStreetMap raster basemap and shows a small note; your 2D tileset selection is unaffected.
- **Switching 2D↔3D keeps your view** (New in 4.14) — the new canvas opens where you were looking, at the same center and zoom, instead of jumping back to your Default Map Center. Pitch and rotation survive a 3D → 2D → 3D round-trip as well, so returning to 3D restores the camera angle you left it at.
- **Follow / Auto-zoom work in 3D** (New in 4.14) — the same toolbar buttons as 2D. Follow recenters on your selected nodes as they move (keeping zoom, pitch, and bearing); Auto-zoom fits their bounds. Panning or rotating the 3D camera pauses following until you click **Resume follow**, exactly as in 2D.
