...
- **Channels tab grew infinitely on installed iOS PWAs**: On the Channels tab in an installed iOS PWA, the message list expanded without bound — the page scrolled forever and the send bar was pushed off the bottom of the screen. `.app-main` is a flex item whose default `min-height: auto` refuses to shrink below its content size; iOS WebKit enforces this strictly, so the message list grew `.app-main` to its full content height instead of clamping it to the viewport (Chrome happened to clamp it anyway, masking the bug). Added `min-height: 0` (and an `overflow: hidden` backstop) to the channels-tab `.app-main` flex container so the conversation is bounded to the viewport and the message list scrolls internally. Regression from the #3307 PWA dead-space fix.
