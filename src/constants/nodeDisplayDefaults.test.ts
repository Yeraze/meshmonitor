...
describe('TX-target window when maxNodeAgeHours is 0 (#5376)', () => {
  ...
  it('SETTINGS_TAB_PER_SOURCE_KEYS = the frozen ten plus the TX-target window', () => {
    expect(SETTINGS_TAB_PER_SOURCE_KEYS).toEqual([...NODE_DISPLAY_SETTING_KEYS, 'txTargetMaxAgeHoursWhenUnlimited']);
    // The frozen ten stay ten (migration 131 seed parity).
    expect(NODE_DISPLAY_SETTING_KEYS).toHaveLength(10);
  });
});
