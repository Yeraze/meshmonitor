...
describe("useProcessedNodes", () => {
  ...
  describe("basic functionality", () => {
    it("should return processed nodes with age filtering", () => {
      ...
      expect(
        result.current.processedNodes.find((n) => n.nodeNum === 11111)
      ).toBeUndefined();
    });
  ...
  describe("text filtering", () => {
    it("should filter by longName", () => {
      ...
      expect(result.current.processedNodes[0].user?.longName).toBe(
        "Alpha Node"
      );
    });
  ...
  describe("advanced filters", () => {
    it("should filter by MQTT in show mode", () => {
      ...
      const { result } = renderHook(
        () => useProcessedNodes({ nodeFilters: filters }),
        { wrapper: createWrapper() }
      );
    ...
    it("should filter by MQTT in hide mode", () => {
      ...
      expect(
        result.current.processedNodes.every((n) => !n.viaMqtt)
      ).toBe(true);
    });
    ...
    it("should filter by power source - powered", () => {
      ...
      const poweredNodes = result.current.processedNodes.filter(
        (n) => n.deviceMetrics?.batteryLevel === 101
      );
  ...
  describe("sorting", () => {
    it("should sort by lastHeard descending", () => {
      ...
      const nonFavorites = result.current.processedNodes.filter(
        (n) => !n.isFavorite
      );
      for (let i = 1; i < nonFavorites.length; i++) {
        expect(nonFavorites[i - 1].lastHeard).toBeGreaterThanOrEqual(
          nonFavorites[i].lastHeard || 0
        );
      }
    });
    ...
    it("should sort by longName ascending", () => {
      ...
      const nodesWithName = nonFavorites.filter(
        (n) => n.user?.longName && n.user.longName.trim() !== ""
      );
    ...
    it("should sort by SNR descending", () => {
      ...
      for (let i = 1; i < nonFavorites.length; i++) {
        expect((nonFavorites[i - 1].snr ?? -999) >= (nonFavorites[i].snr ?? -999)).toBe(
          true
        );
      }
    });
  ...
  describe("option overrides", () => {
    it("should use maxNodeAgeHours override", () => {
      ...
      const { result } = renderHook(
        () => useProcessedNodes({ maxNodeAgeHours: 1 }),
        { wrapper: createWrapper() }
      );
    ...
    it("should use textFilter override", () => {
      const { result } = renderHook(
        () => useProcessedNodes({ textFilter: "Delta" }),
        { wrapper: createWrapper() }
      );
