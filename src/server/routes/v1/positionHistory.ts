...
router.get('/:nodeId/position-history', async (req: Request, res: Response) => {
  try {
    ...
    // Fetch position telemetry - use a larger internal limit to account for grouping
    // Each position has up to 5 telemetry rows (lat, lon, alt, speed, track)
    const internalLimit = maxLimit * 5;
