 ...
 * Uses session/cookie authentication (for UI access).
 */
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    // Only admins can access channel database
    if (!isAdmin) {
...
/**
 * GET /api/channel-database/retroactive-decrypt/progress
...
/**
 * PUT /api/channel-database/reorder
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.enforceNameValidation !== undefined) {
      updates.enforceNameValidation = Boolean(req.body.enforceNameValidation);
    }
...
/**
 * DELETE /api/channel-database/:id
...
/**
 * POST /api/channel-database/:id/retroactive-decrypt
