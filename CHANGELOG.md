...
- **Channels tab layout broken on installed iOS PWAs (infinite scroll + dead space)**: On the Channels tab in an installed iOS PWA the message list either grew without bound — the page scrolled forever and the send bar was pushed off-screen — or, with a short message list, left a large dead-space gap below the send bar. Both stem from iOS WebKit being strict about flexbox sizing where Chrome is lenient (which masked the bug locally): (1) `.app-main`'s default `min-height: auto` refused to shrink below its content, so the message list expanded it past the viewport — fixed by adding `min-height: 0` plus an `overflow: hidden` backstop; (2) the growing flex children (`.channels-tab-content`, `.messages-container`) used `flex: 1 1 auto`, and iOS only reliably grows a flex item to fill free space when its basis is `0` — switched them to `flex: 1 1 0` so the conversation fills the viewport and the send bar stays pinned at the bottom; (3) the home-indicator safe-area inset was counted twice — once on `.app-main`'s `padding-bottom` and again on `.send-message-form` — leaving a residual gap below the input, so it is now dropped from `.app-main` on the channels tab and owned solely by the send bar. (1) and (2) were invisible on Chrome/desktop because `env(safe-area-inset-bottom)` is `0` there. Regression from the #3307 PWA dead-space fix.
