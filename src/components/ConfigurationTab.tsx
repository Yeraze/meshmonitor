...
const ConfigurationTab: React.FC<ConfigurationTabProps> = ({ nodes, channels = [], onRebootDevice, onConfigChangeTriggeringReboot, onChannelsUpdated, refreshTrigger }) => {
    ...
    const fetchConfig = async () => {
      ...
      try {
        ...
        if (config.moduleConfig?.meshBeacon) {
          ...
          // Interval + targets (#4802). Interval absence reads as the firmware
          // minimum (3600), not 0.
  ...
  const handleSaveMeshBeaconConfig = async () => {
    ...
    setStatusMessage('');
    try {
      // Shared builder owns the wire omit-logic (offer channel, optional
      // preset, target normalisation) — see buildMeshBeaconConfigPayload.
