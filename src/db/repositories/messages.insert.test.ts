...
describe('MessagesRepository.insertMessage duplicate detection', () => {
  ...
  it('persists the xeddsaSigned flag (#3923)', async () => {
    await repo.insertMessage({ ...makeMessage('msg-signed'), xeddsaSigned: true } as any, 'default');
    await repo.insertMessage({ ...makeMessage('msg-unsigned'), xeddsaSigned: undefined } as any, 'default');
    ...
    const rows = await repo.getMessages(50, 0, 'default');
