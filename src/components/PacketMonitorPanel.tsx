...
const PacketMonitorPanel: React.FC<PacketMonitorPanelProps> = ({ onClose, onNodeClick, standalone }) => {
  ...
  // Locale-aware wrappers around the shared pure formatters.
  const formatDateColumn = (timestamp: number): string =>
    formatPacketDateColumn(timestamp, dateFormat, t('packet_monitor.today', 'Today'));
  const formatTimestamp = (timestamp: number): string =>
    formatPacketTimestamp(timestamp, timeFormat);
