...
- **~~One caveat on `:dev`.~~** ~~Right after a stable release ships, `:dev` may briefly point at an image that's *older* than `:latest` — the most recent RC that preceded the stable cut — until the next RC is published. This is expected and self-corrects the moment the next RC lands.~~<br />**Update (2026-07-28): fixed — this caveat no longer applies.** Stable releases are now published to `:dev` as well as `:latest`, so the fast track is always a superset of the stable track. `:dev` is never older than `:latest`, and never moves backwards.
