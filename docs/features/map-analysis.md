...
| **Transport filter (Show RF / UDP / MQTT)** | Hides markers **and, since 4.16.0, route segments** by how the traffic reached MeshMonitor — over the air (**RF**), Meshtastic's multicast-UDP transport (**UDP**), or an **MQTT** broker or bridge. Matches the same toggle on the Dashboard and Nodes maps. A node heard over more than one transport stays visible while any of them is on. See [Transport filtering and route segments](/features/maps#transport-filtering-and-route-segments). |
