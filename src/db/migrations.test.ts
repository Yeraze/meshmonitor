...
describe('migrations registry', () => {
  it('has all 75 migrations registered', () => {
    expect(registry.count()).toBe(75);
  });
  ...
  it('last migration is meshcore_packet_log', () => {
    const all = registry.getAll();
    const last = all[all.length - 1];
    expect(last.number).toBe(75);
    expect(last.name).toContain('meshcore_packet_log');
  });

  it('migrations are sequentially numbered from 1 to 75', () => {
    const all = registry.getAll();
