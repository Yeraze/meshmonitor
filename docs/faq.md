...
- **MeshBeacon.** 2.8 adds a periodic `MESH_BEACON_APP` broadcast that a node can use to advertise a joinable channel (name, region, and modem preset). The Packet Monitor decodes and previews these as `[MeshBeacon: "..."]`. You can also:
  - **Configure the module** from Configuration → MeshBeacon, or the Admin Commands tab: listen and broadcast toggles, the beacon text, the broadcast interval, what network (if any) to advertise, and a **Broadcast Targets** list naming where the beacon is transmitted. Each target picks a modem preset, a region, and one of the node's own channel slots; leave the list empty to send a single beacon on the node's running preset and region over the primary channel. Firmware accepts at most four targets and 100 bytes of beacon text — go over either and the device discards the whole MeshBeacon config without reporting an error, so MeshMonitor blocks both in the editor. The section is greyed out on firmware older than 2.8, because such a device silently ignores the setting rather than rejecting it.
...
Apart from the MeshBeacon module settings, none of this needs any action from you, it's decode support so MeshMonitor stays useful against nodes running early 2.8 builds. Expect this section to be replaced with normal release documentation once 2.8 ships.
