/**
 * Migration 101: Consolidate MQTT channels by name (issue: channels split by
 * sending slot/hash).
 *
...
export const migration = {
  up: (db: Database): void => {
    logger.info('Running migration 101 (SQLite): consolidating MQTT channels by name...');
    ...
    logger.info(
      `Migration 101 complete (SQLite): merged ${mergedDbRows} duplicate channel_database row(s), ` +
      `collapsed ${collapsedChannelRows} MQTT channel row(s), repointed ${repointedMessages} message(s).`,
    );
  },

  down: (_db: Database): void => {
    logger.debug('Migration 101 down: not implemented (data consolidation is not reversible)');
  },
};
...
export async function runMigration101Postgres(client: import('pg').PoolClient): Promise<void> {
  logger.info('Running migration 101 (PostgreSQL): consolidating MQTT channels by name...');
  ...
  logger.info(
    `Migration 101 complete (PostgreSQL): merged ${mergedDbRows} duplicate channel_database row(s), ` +
    `collapsed ${collapsedChannelRows} MQTT channel row(s), repointed ${repointedMessages} message(s).`,
  );
}
...
export async function runMigration101Mysql(pool: import('mysql2/promise').Pool): Promise<void> {
  logger.info('Running migration 101 (MySQL): consolidating MQTT channels by name...');
  ...
  logger.info(
    `Migration 101 complete (MySQL): merged ${mergedDbRows} duplicate channel_database row(s), ` +
    `collapsed ${collapsedChannelRows} MQTT channel row(s), repointed ${repointedMessages} message(s).`,
  );
}
