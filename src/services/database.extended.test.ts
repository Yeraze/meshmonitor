...
const createTestDatabase = (sqlite: Database.Database) => {
  class TestDatabaseService {
    ...
    getNodeNeedingTraceroute(localNodeNum: number): DbNode | null {
      ...
      // #4412 Phase 3 (WP5): this fake used to re-implement the production
      // maxNodeAgeHours active-node cutoff via a bare, un-namespaced
      // `this.getSetting('maxNodeAgeHours')` read — the exact pre-Phase-2
      // global read production no longer does (it now resolves per-source
      // via getMaxNodeAgeHours). That branch, and the three tests that
      // asserted against this fake's own re-implementation rather than
      // production behavior, are gone; real per-source coverage for
      // getNodeNeedingTracerouteAsync lives in
      // src/services/database.maxNodeAge.perSource.test.ts. This fake no
      // longer filters by node age at all — no remaining test in this file
      // depends on that filtering (every fixture here uses a fresh
      // `lastHeard`).
      const stmt = this.db.prepare(`
        SELECT n.*,
...
describe('DatabaseService - Extended Coverage', () => {
  ...
  describe('Traceroute Node Selection', () => {
    ...
    // The three maxNodeAgeHours tests formerly here (#4412 Phase 3 WP5 /
    // spec §2.3) asserted against this file's own fake `getNodeNeedingTraceroute`
    // re-implementation of the *pre-Phase-2* global `maxNodeAgeHours` read —
    // they would have passed identically if production had reverted to a
    // global read. Production resolves the window per-source via
    // `getMaxNodeAgeHours` (getNodeNeedingTracerouteAsync); real coverage for
    // that lives in src/services/database.maxNodeAge.perSource.test.ts.
  });
