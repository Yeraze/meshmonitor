/**
 * Migration 003: Add 'connection' to permissions resource CHECK constraint
 *
 * Adds the 'connection' resource type to the permissions table CHECK constraint
 */
...
export const migration = {
  up: (db: Database): void => {
    ...
    try {
      ...
      logger.debug('✅ Migration 003 completed: connection resource added');
    } catch (error) {
  ...
  down: (db: Database): void => {
    logger.debug('Running migration 003 down: Remove connection permission resource');

    try {
      // Remove connection permissions first
      db.exec(`DELETE FROM permissions WHERE resource = 'connection'`);

      // Recreate table without connection in CHECK constraint
      db.exec(`
        CREATE TABLE permissions_new (
          id INTEGER PRIMARY KEY AUTOINCREMENT,
          user_id INTEGER NOT NULL,
      ...
      logger.debug('✅ Migration 003 rollback completed');
    } catch (error) {
