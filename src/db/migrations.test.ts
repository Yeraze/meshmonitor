...
describe('migrations registry', () => {
  it('has all 81 migrations registered', () => {
  ...
  it('last migration is add_sources_display_order', () => {
    const all = registry.getAll();
    const last = all[all.length - 1];
    expect(last.number).toBe(81);
    expect(last.name).toContain('add_sources_display_order');
  });

  it('migrations are sequentially numbered from 1 to 81', () => {
    const all = registry.getAll();
